...
export async function main(ns) {
  ...
  */
  ...
  // Find out how many threads the main host sever can handle
  const mainHostRAM = ns.getServerMaxRam(mainHost);
  const scriptRAMUsage = ns.getScriptRam("grow.js");
  const mainHostCapacity = mainHostRAM / scriptRAMUsage;
  ...
  while (true) {
    ...
    while (targetCurrentMoney < (targetMaxMoney * ((moneyToStealPerBatch * 3) / 100)) || growThread > mainHostCapacity) {
      // time refresh
      ...
      for (let i = 0; i < ratioWknGrw; i++) {
        if (growThread > 1) {
          ns.exec("grow.js", prepHost, Math.ceil(growThread / 1), target);
